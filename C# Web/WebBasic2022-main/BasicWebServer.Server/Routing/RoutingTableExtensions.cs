...
namespace BasicWebServer.Server.Routing
{
    public static class RoutingTableExtensions
    {
        public static IRoutingTable MapGet<TController>(
            ...
            => routingTable.Map(Method.Get, path, request
        ...
        public static IRoutingTable MapPost<TController>(
            ...
            Func<TController, Response> controllerFunction)
            where TController : Controller
            => routingTable.Map(Method.Post, path, request
                => controllerFunction(CreateController<TController>(request)));

        public static IRoutingTable MapControllers(this IRoutingTable routingTable)
        {
            IEnumerable<MethodInfo> controllerActions = GetControllerActons();
            ...
            => request =>
        {
            ...
            var parameteresValues = GetParametersValues(controllerAction, request); 

            return (Response)controllerAction.Invoke(controllerInstance, parameteresValues);
        };

        private static object[] GetParametersValues(MethodInfo controllerAction, Request request)
        {
            var actionsParameters = controllerAction
                .GetParameters()
                .Select(p => new
                {
                    p.ParameterType,
            ...
            var parameterValues = new object[actionsParameters.Length];

            for (int i = 0; i < actionsParameters.Length; i++)
            {
                var parameter = actionsParameters[i];
        ...
        private static IEnumerable<MethodInfo> GetControllerActons()
                => Assembly
            .GetEntryAssembly()
            .GetExportedTypes()
        ...
        {
            var controller = (Controller)Request.ServiceCollection.CreateIstannce(controllerType);
