...
namespace _11._Array_Manipulator
{
    class Program
    {
        ...
        {
            int[] firstArray = new int[arrey.Length - index - 1];
            int[] secondArray = new int[index + 1];

            int firstArrayCouner = 0;

            for (int i = index + 1; i < arrey.Length; i++)
            {
                firstArray[firstArrayCouner] = arrey[i];
                firstArrayCouner++;
            }

            for (int i = 0; i < index + 1; i++)
            {
                secondArray[i] = arrey[i];
            }

            for (int i = 0; i < firstArray.Length; i++)
            {
                arrey[i] = firstArray[i];
            }

            for (int i = 0; i < secondArray.Length; i++)
            {
                arrey[firstArray.Length + i] = secondArray[i];
            }
        }
        ...
        static void ReturnFirstEven(int[] array, int count)
        {
            ...
            var result = number.Split(' ', StringSplitOptions.RemoveEmptyEntries);
